/**
 * Script de calcul de DD Tech (Due Diligence) pour un ou plusieurs éditeurs
 * 
 * Usage: 
 *   npm run calculate-dd-tech -- "Éditeur 1" "Éditeur 2" ...
 *   ou
 *   node dist/scripts/calculateDDTech.js "Éditeur 1" "Éditeur 2"
 * 
 * Ce script:
 ...
 * 6. Crée des ScoringSnapshots avec collection_type='DD'
 */
...
/**
 * Calcule et enregistre un snapshot DD Tech pour une solution
 */
async function calculateDDTechForSolution(
  solutionId: mongoose.Types.ObjectId,
  solutionName: string,
  ...
  try {
    ...
    if (missingFields.length > 0) {
      console.warn(`\n⚠️  Impossible de calculer le score DD Tech pour "${solutionName}" - Données manquantes:`);
    ...
    try {
      ...
      console.warn(`⚠️  Impossible de calculer le score DD Tech pour "${solutionName}": ${error.message}`);
      return false;
    }

    if (!tempSnapshot) {
      console.warn(`⚠️  Impossible de calculer le score DD Tech pour "${solutionName}" - données manquantes`);
      return false;
    }
    ...
    // Créer le snapshot avec le bon collection_type='DD' et scoreId
    ...
    console.log(`✅ DD Tech créé pour "${solutionName}" - Score: ${newSnapshot.global_score}, Risque: ${newSnapshot.risk_level}`);
    return true;
  } catch (error: any) {
    console.error(`❌ Erreur lors du calcul DD Tech pour "${solutionName}":`, error.message);
    return false;
  }
}
 ...
 * Fonction principale
 */
async function calculateDDTech() {
  try {
    ...
    if (editorNames.length === 0) {
      ...
      console.log('Usage: npm run calculate-dd-tech -- "Éditeur 1" "Éditeur 2" ...');
      console.log('Exemple: npm run calculate-dd-tech -- "GI Informatique" "Cogima"');
      process.exit(1);
    }
    ...
    // Pour chaque éditeur, trouver les solutions et calculer les DD Tech
    console.log(`\n📊 Calcul des DD Tech...\n`);

    let totalSolutions = 0;
    let successfulDDTech = 0;
    let failedDDTech = 0;
    ...
    for (const editor of editors) {
      ...
      for (const solution of solutions) {
        totalSolutions++;
        const success = await calculateDDTechForSolution(
          solution._id,
          solution.name,
          'DD'
        );
    ...
    console.log(`   DD Tech créés avec succès: ${successfulDDTech}`);
    console.log(`   Échecs: ${failedDDTech}`);
    console.log('='.repeat(80) + '\n');
    ...
    console.error('\n❌ Erreur lors du calcul des DD Tech:', error.message);
    console.error(error);
    await mongoose.connection.close();
    process.exit(1);
  }
